...
def process_text(text):
    ...
    for sent in doc_text.sents:
        for token in sent:
            ...
            if token.is_oov:  # обработка опечаток
                print("Опечатка " + str(token_in_vocabulary.text))
                suggestions = hspell.suggest(token.text)
                word = suggestions[0] if suggestions else token.text
                doc_word = nlp(word)
...
def process_large(text):
    text = clean_text(text)
    text = re.sub(r'\d+', '0', text)

    def split_text_into_chunks(text, words_per_chunk=500000):
...
def full_process_text(filename):
    try:
        ...
        path_out = os.path.join('D:\\Work\\projects\\pycharm\\results', f'lemmatized_{filename}')
